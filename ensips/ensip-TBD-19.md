...
This ENSIP proposes a new dedicated record type for storing arbitrary data in ENS names. This proposal introduces a purpose-built interface with the following functions:

```
function data(bytes32 node, bytes calldata key) external view returns (bytes memory);
function setData(bytes32 node, bytes calldata key, bytes calldata data) external;
```
...
To store data, the function `setData` is used: 

```
setData(node, bytes("aiContext"), hex"0001ABCD...");
```

To retrieve data, the function `data` is used:

```
bytes memory result = data(node, bytes("aiContext"));
```
...
```
event DataChanged(bytes32 indexed node, bytes indexed key, bytes data);
```
...
ENS names have become widely used as identifiers across various applications and protocols, with growing demand for storing additional metadata and arbitrary data that doesn't fit into existing record types. This ENSIP introduces a dedicated `data` record type using bytes-to-bytes key-value pairs, providing maximum flexibility for key generation strategies and ensuring the record type can adapt to various use cases including hashes, metadata storage, content references, low-resolution images, raw byte codes, and serialized data structures. For most applications, data can be stored offchain or in content-addressable storage systems (e.g., IPFS or Arweave), with only references stored onchain, while this ENSIP intentionally leaves data encoding unspecified to accommodate future use cases.
